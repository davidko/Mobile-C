...
The `-DCMAKE\_INSTALL\_PREFIX' option can be used to specify the home directory to 
install the Mobile-C files, as shown in the following commands.

\begin{verbatim}
    cd <MCPACKAGE>
    mkdir build
    cmake .. -DCMAKE_INSTALL_PREFIX=<MCHOME>
    make
    make install
\end{verbatim}
...
\item Type the command \texttt{mkdir build}
\item Type the command \texttt{cd build}
\item Generate the build files with the command \texttt{cmake .. -G "NMake
    Makefiles"}
\item Type the command \texttt{nmake} to build the Mobile-C
